...
def train(args):
    ...
    # 输出到文件
    file_handler = logging.FileHandler(args.log_path + timestamp + '.txt')
    ...
    util = data_utils.UtilForCrossValidation(bert_path=args.bert_path,
                                             max_seq_len=args.max_seq_len,
                                             batch_size=args.batch_size,
                                             label_dict_path='./data/label_dict.json')
    for fold in range(args.num_fold):
        logger.info(10 * '*' + '{}/{}'.format(fold + 1, args.num_fold) + '*' * 10)
        model_name = args.model_name.format(fold)
        # 数据加载
        train_loader = util.loader('train', fold)
        ...
        for epoch in range(args.epochs):
            ...
            if best_score < dev_f1:
                ...
                torch.save(model.state_dict(), args.model_save_path + model_name)
                best_score = dev_f1
                patience = 0
            else:
...
def predict(args):
    ...
    for fold in range(args.num_fold):
        model_name = args.model_save_path + args.model_name.format(fold)
...
if __name__ == "__main__":
    ...
    if platform.system() == 'Windows':
        bert_path = r'../pytorch_chinese_L-24_H-1024_A-16/'
    else:
    ...
    parser.add_argument("--accumulation_steps", type=int, default=6, help="梯度累加")
    ...
    parser.add_argument('--model_name', type=str, default='bert_large_{}.bin', help='模型名称')
